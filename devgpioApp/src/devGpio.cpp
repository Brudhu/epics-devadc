...
long devGpioInitRecord( dbCommon *prec, devGpio_rec_t* pconf ){
  ...
  try{
    ...
    // Adcs are always input:
    if( pconf->output ) {
      //GpioManager::instance().setDirection( gpioID, GpioManager::OUTPUT );
        std::cerr << prec->name << ": " << "Adcs are always input, not output";
        return ERROR;
    }
...
extern "C" {
  ...
  static const iocshFuncDef initFuncDef = { "AdcConstConfigure", 1, initArgs };
  ...
  epicsExportRegistrar( devAdcConstRegister );
}
